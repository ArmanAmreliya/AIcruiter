...
test.describe("AIcruiter Landing Page E2E Tests", () => {
  ...
  test("should load the homepage and display key elements", async ({
    ...
    // 2. Check the updated hero copy
    const heading = page.locator("h1").first();
    await expect(heading).toContainText("Your");
    await expect(heading).toContainText("AI interviewer");
    await expect(heading).toContainText("for every");
    await expect(heading).toContainText("candidate screen");

    // 3. Check for the current hero badge
    const badge = page.getByText(/AI voice screening for modern hiring teams/i);
    await expect(badge).toBeVisible();

    // 4. Verify the primary hero CTA is present
    const getStartedButton = page
      .getByRole("button", { name: /Start screening smarter/i })
      .first();
  ...
  test("should navigate to signup page when clicking Get Started", async ({
    ...
    const signUpButton = page
      .locator('nav button:has-text("Get Started")')
      .first();
